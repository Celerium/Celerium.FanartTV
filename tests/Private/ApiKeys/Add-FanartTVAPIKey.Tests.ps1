...
Describe "Testing the [ $buildTarget ] version of [ $commandName ] functions with [ $pester_TestName ]" -Tag @('apiKeys') {
    ...
    Context "[ $commandName ] testing functions" {
        ...
        It "[ $commandName ] should have an alias of [ Set-FanartTVAPIKey ]" {
            Get-Alias -Name Set-FanartTVAPIKey | Should -BeTrue
        }

        It "[ -PersonalKey ] should not be null" {
            Add-FanartTVAPIKey -PersonalKey 12345
            (Get-FanartTVAPIKey).PersonalKey | Should -Not -BeNullOrEmpty
        }

        It "[ -ProjectKey ] should not be null" {
            Add-FanartTVAPIKey -ProjectKey 12345
            (Get-FanartTVAPIKey).ProjectKey | Should -Not -BeNullOrEmpty
        }
